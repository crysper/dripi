  ...
  module Generators
    class InstallGenerator < Rails::Generators::Base
      ...
      def run_other_generators
        ...
        generate "model Drip::Sequence dripable_id:integer:index dripable_type:string scheduled_job_id:string scheduled_at:datetime paused_at:datetime current_id:integer:index"
...
class Drip::Item < ApplicationRecord
  ...
  def execution_test?(current_triggerer,sequence,extra)
    drip_starter=sequence.drip_starter
    #specify conditions to test if the current sequence action should be executed(after delay is over)

  end

  def execute_action!(current_triggerer,sequence,extra)
    drip_starter=sequence.drip_starter
    #the current sequence action to be executed. When this returns true the sequence will proceed to the next item and wait for the trigger
      ...
      if File.exists?(drip_job_file)
        inject_into_file drip_job_file, :after => "perform(*args)" do
          %Q{
    item=Dripi._obj(item_) #deserializing Drip::Item, you can use Dripi._obj to deserialize the other args as well
    item.execute_(current_triggerer_,sequence_,extra_) if item  #executing job if drip item found, calls execution_test? then execute_action! from Drip::Item
          }
        end
